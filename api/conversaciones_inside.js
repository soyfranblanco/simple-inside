export default async function handler(req, res) {
  ...
  const { action, email, id, mensajes, resumen } = req.body;
  try {
    if (action === "get") {
      if (!email) return res.status(400).json({ error: "Falta email" });
      const r = await fetch(`${SUPABASE_URL}/rest/v1/conversaciones_inside?usuario_email=eq.${encodeURIComponent(email)}&order=updated_at.desc&limit=1&select=*`, { headers });
    ...
    if (action === "insert") {
      ...
      const body = { usuario_email: email, mensajes };
      if (resumen) body.resumen = resumen;
    ...
    if (action === "update") {
      ...
      const body = { mensajes, updated_at: new Date().toISOString() };
      if (resumen) body.resumen = resumen;
